/*
The goal of this project is to make Cam's Pizzeria website run jank-free at 60
frames per second.

Major issues:
1. Expensive document.querySelectorAll method is being called repeatedly in a loop.
2. Repeatedly variable calcuation in a loop is redundant
3. 200 animating pizzas are rendered at any given time but less than 50 of them
are visiable.
...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    // store DOM to a temp variable
    var pizzaSizeInnerHTML = document.querySelector("#pizzaSize").innerHTML;
    switch(size) {
      case "1":
...
function updatePositions() {
  ...
  // to avoid repeated calcuation in the loop
  var phaseArray = [];
  // For loop to generate five fixed phases
  for (var index = 0; index < 5; index++){
    phaseArray.push(Math.sin(documentBodyScrollTop + index));
  }
  // For loop to assign phases to items
